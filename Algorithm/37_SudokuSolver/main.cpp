...
public:
    ...
    vector<char> getNumber(vector<vector<char>> &board, int x, int y)
    {
        vector<char> number;
        for (int i = 0; i < sizeX; i++)
        {
            char target = charList[i];
            if (isValida(board, x, y, target))
            {
                number.push_back(target);
            }
        }
        return number;
    }
    bool solve(vector<vector<char>> &board, int x, int y)
    {
        ...
        {
            vector<char> numbers = getNumber(board, x, y);
            int nextY = (y + 1) % sizeY;
            int nextX = nextY == 0 ? x + 1 : x;
            for (int i = 0; i < numbers.size(); ++i)
            {
                board[x][y] = numbers[i];
                bool isValid = solve(board, nextX, nextY);
                if (isValid)
                {
                    return true;
                }
